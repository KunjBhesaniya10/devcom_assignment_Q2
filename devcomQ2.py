...
class SyncService:   #syncservice implementation --->
    def __init__(self):
        self.combined__records=[]
    ...
    def onMessage(self, data: dict):
       ...
       #onMessage function ------------------>
        if not data:
            return
        if data['type'] =='record':
           self.combined__records.append(data)
          
        elif data['type'] == 'probe':
            _data = { 'type':'update', 'from': data['from'], 'data':self.combined__records[data['from']:] } 
            return _data    
